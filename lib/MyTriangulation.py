...
class Triang():
    ...
    def flann(self):
        ...
        for i in self.idsL:
            ...
            colours.append([red, green, blue, label]) #Append to the colours' list the colours as well as the associated labels.
    ...
    def calculate_fundamental_matrix(self):
        ...
        self.fundamental_matrix, mask = cv.findFundamentalMat(self.ptsL, self.ptsR, cv.FM_LMEDS) #Calculate fundametnal matrix (Unknown camera intrinsics parameters).
        
        self.ptsL = self.ptsL[mask.ravel()==1] #Keep only the points which are used for the fundametnal matrix calculation.
        self.ptsR = self.ptsR[mask.ravel()==1]
        self.colours = self.colours[mask.ravel()==1] #Keep only the colours and the labels from points which are used for the fundametnal matrix calculation.
    ...
    def calculate_essential_matrix(self):
        ...
        self.ptsL    = self.ptsL[mask.ravel()==1] #Keep only the points which are used for the essential matrix calculation.
        self.ptsR    = self.ptsR[mask.ravel()==1]
        self.colours = self.colours[mask.ravel()==1] #Keep only the colours and the labels of points which are used for the essential matrix calculation.
    ...
    def Rt(self):
        ...
        self.ptsL = self.ptsL[mask.ravel()==255] #Keep only the points which are used for the pose recover procedure.
        self.ptsR = self.ptsR[mask.ravel()==255]
        self.colours = self.colours[mask.ravel()==255] #Keep only the colours of points which are used for the pose recover calculation.
    ...
    def projection_matrix_from_pose(self):
        ...
        self.left_projection_matrix = np.matmul(np.array(self.camera_matrix), P_t) #Calculate projection matrix from pose.
    ...
    def starting_projection_matrix(self):
        ...
        self.right_projection_matrix = np.concatenate(self.right_projection_matrix, axis=1) #Set the start projection matrix.
    ...
    def triangulate_points(self):
        ...
        self.triangulated_points = cv.triangulatePoints(np.array(self.right_projection_matrix), np.array(self.left_projection_matrix), ptsRT, ptsLT) #Calculate 4D points (x, y, z, w) from triangulation 
        self.triangulated_pointsT = np.transpose(self.triangulated_points) #Find the transpose of triangulated points list

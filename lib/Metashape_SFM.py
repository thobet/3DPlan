...
class MetaSFM():
    ...
    def sfmmvs(self):
        ...
        if Metashape.app.activated == True:
            ...
            self.doc.chunk.exportPoints(path=f'{self.path}/Lines/merged.ply', binary=False, save_normals=False, save_colors=True, colors_rgb_8bit=False)
            self.doc.save(path)
        else:
            error_message('Project was not saved due to deactivated license. Please activate your Agisoft Metashape License and try again.', sysex=True)
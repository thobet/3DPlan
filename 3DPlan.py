...
if __name__ =='__main__':
    ...
    out = ['3D', '4D']
    out_selection = input_check('3D (0) or 4D (1) output? (Write 0 or 1): ', [0, 1], 'Not a valid answer, please try again   ')
    if out_selection == 0:
    ...
    SFM_selection = input_check('Agisoft-Metashape (0), Mapillary-OpenSFM (1) or MyTriangulation (2)? (Write 0, 1 or 2): ', [0, 1, 2], 'Not a valid answer, please try again')
    if SFM_selection == 0:
        message('Agisoft-Metashape option is selected.')
        agi_selection = input_check('Python Module (0) or GUI (1) output? (Write 0 or 1): ', [0, 1], 'Not a valid answer, please try again')
    ...
    semantic_selection = input_check('Canny (0) or external semantic information (1)? (Write 0 or 1): ', [0, 1], 'Not a valid answer, please try again')
    
    imgsuff = input_check('Give the available image format: ', ['.JPG', '.jpg', '.TIFF', '.tiff', '.tif', '.PNG', '.png'], 'The given format is not available or valid.')
    images = find_files(f'{path}/rgb', imgsuff)
    
    if len(images) == 0:
        ans = input_check(f'There are 0 images into rgb directory with {imgsuff} format do you want to continue the process? (y, n) ', ['y', 'Y', 'n', 'N'], 'Not a valid answer, please try again')
        if ans == 'n' or ans == 'N':
    ...
    if semantic_selection == 0:
        message('Canny option was selected. Set the min and max values, using the trackbars, and then press Q to edit the next image.')
        for image in images:
    ...
    if sfm == 'Agisoft_Metashape':
        ...
        if agi_selection == 0:
            ...
            message('GUI was selected. Use the 4D images from the (./3DPlan/images) path for the dense cloud production using Agisoft Metashape GUI. When the dense cloud is produced, export it as "merged.txt" in the (./3DPlan/Lines) directory.')
            out = input_check('If the produced point cloud i.e., merged.txt is added into ./3DPlan/Lines/ directory write 1 and then press enter to continue: ', [1], 'Try again, not a valid answer')
